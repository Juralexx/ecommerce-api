...
/**
 * Return a random ID containing numbers.
 * @param {*} max Length of the required ID
 */
...
export const generateStrongPassword = (max) => {
    const allCapsAlpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split('');
    const allLowerAlpha = "abcdefghijklmnopqrstuvwxyz".split('');
    const allUniqueChars = "~!@#$%^&*()_+-=[]\\{}|;:,./<>?".split('');
    const allNumbers = "0123456789".split('');
    ...
    const charsTypes = [[...allCapsAlpha], [...allLowerAlpha], [...allUniqueChars], [...allNumbers]]

    let password = ''
    charsTypes.map((charstype) => {
        return password += generator(charstype, 50)
    })

    const passwordArr = password.split('')
    const randomized = shuffleArray(passwordArr)
...
/**
 * Return a number between the min and max range
